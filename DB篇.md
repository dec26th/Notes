...
- **MySql与redis**
  ...
  - 缓存出现的原因
    - 在网络处理和访问非常大的时候，数据库的压力就会变大，数据库的连接吃，数据库同时处理数据的能力就会收到很大的挑战。而高并发处理，分布式，负载均衡的数据库又比较的耗费人力以及财力。
  - 实际流程
    - 在实际的流程当中，Redis与mysql进行了读写分离。在执行查询语句的时候，首先会计算该语句的MD5并且根据这个来获取结果集的标识符，然后利用标识符去redis中查找该结果集。若redis中没有相应的存储内容，则会去查找mysql。并且将结果集中的元素在写入到redis中去。
  - 其他的缓存策略
    - Memcache与redis
      - 性能上
      ...
      - 内存空间和数据量大小
        - memcache可以修改最大内存，采用LRU算法。而Redis增加了VM的特性，突破了物理内存的限制。
      - 操作便利
